...
PasswordCodeResponseErrors
LoginManager::sendEmailCode(const std::string email, unsigned int code) const {
...
LoginManager::resetPassword(const std::string &username,
                            const std::string &newPassword) const {
  ResetPasswordResponseErrors resetPasswordErrors;
  if (!RegexValidator::validUsername(username)) {
  ...
  if (!this->m_database.doesUserExist(username)) {
    resetPasswordErrors.generalError =
        std::string() + "Unknown username: " + username;
  }
