...
RequestResult MenuRequestHandler::handleRequest(const RequestInfo &requestInfo) const
{
    ...
    catch (const std::exception &e)
    {
        ...
        RequestResult res(JsonResponsePacketSerializer::serializeResponse(errResponse), nullptr);
        return res;
    }
}
...
RequestResult MenuRequestHandler::logout(const RequestInfo &info) const
{
    ...
    RequestResult requestResult(JsonResponsePacketSerializer::serializeResponse(logOutResponse),
...
RequestResult MenuRequestHandler::getRooms(const RequestInfo &requestInfo) const
{
    ...
    RequestResult requestResult(JsonResponsePacketSerializer::serializeResponse(getRoomsResponse),
...
RequestResult MenuRequestHandler::getPlayersInRoom(const RequestInfo &requestInfo) const
{
    ...
    RequestResult requestResult(JsonResponsePacketSerializer::serializeResponse(getPlayersInRoomResponse),
                                std::move(this->m_handlerFactory.createMenuRequestHandler(this->m_user)));
    return requestResult;
}
...
RequestResult MenuRequestHandler::getPersonalStats(const RequestInfo &requestInfo) const
{
    ...
    RequestResult requestResult(JsonResponsePacketSerializer::serializeResponse(personalStatsResponse),
                                std::move(this->m_handlerFactory.createMenuRequestHandler(this->m_user)));
    return requestResult;
}
...
RequestResult MenuRequestHandler::getHighScore(const RequestInfo &requestInfo) const
{
    ...
    RequestResult requestResult(JsonResponsePacketSerializer::serializeResponse(highScoreResponse),
                                std::move(this->m_handlerFactory.createMenuRequestHandler(this->m_user)));
    return requestResult;
}
...
{
    JoinRoomRequest request = JsonRequestPacketDeserializer<JoinRoomRequest>::deserializeRequest(requestInfo.buffer);

    int id = request.getRoomId();
    RoomManager &roomManager = m_handlerFactory.getRoomManger();
    JoinRoomResponseErrors joinRoomResponseErrors = m_handlerFactory.getRoomManger().joinRoom(id, this->m_user);
    JoinRoomResponse joinRoomResponse(&joinRoomResponseErrors);
    RequestResult requestResult(JsonResponsePacketSerializer::serializeResponse(joinRoomResponse),
                                std::move(this->m_handlerFactory.createMenuRequestHandler(this->m_user)));
    return requestResult;
}
...
RequestResult MenuRequestHandler::createRoom(const RequestInfo &requestInfo) const
{
    ...
    RequestResult requestResult(JsonResponsePacketSerializer::serializeResponse(createRoomResponse),
                                std::move(this->m_handlerFactory.createMenuRequestHandler(this->m_user)));
    return requestResult;
}
